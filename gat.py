...
class Model(object):
    ...
    def __init__(self, placeholders, input_dim):
        ...
        self.M = tens((FLAGS.node_num, FLAGS.feature_dim), name='mask')
    ...
    def build(self):
        ...
        self.loss_explainer += tf.reduce_mean(
            tf.losses.log_loss(labels=self.placeholders['labels'], predictions=self.outputs))
    ...
    def _build(self):
        ...
        self.gat_layers.append(gat_layer(input_dim=FLAGS.hidden1_gat * FLAGS.attn_heads, F_=FLAGS.output_gat,
                                         placeholders=self.placeholders,
                                         attn_heads=3, attn_heads_reduction='average',
                                         activation=tf.nn.leaky_relu, use_bias=True, name_='2'))

        self.fc_layers.append(
            fc_layer(input_dim=FLAGS.output_gat, output_dim=FLAGS.output_dim, placeholders=self.placeholders,
                     act=tf.nn.sigmoid, dropout=True, name_='1'))

        self.fc_layers.append(
            fc_layer(input_dim=FLAGS.node_num, output_dim=FLAGS.node_num, placeholders=self.placeholders,
...
flags.DEFINE_integer('node_num', 200, 'Number of Graph nodes')
...
flags.DEFINE_integer('batch_num', 12, 'Number of epochs to train')
flags.DEFINE_integer('epochs', 200, 'Number of epochs to train')
flags.DEFINE_integer('attn_heads', 5, 'Number of attention head')
flags.DEFINE_integer('feature_dim', 400, 'Dimensions of features')

flags.DEFINE_integer('hidden1_gat', 32, 'Number of units in hidden layer 1 of gcn')
...
#data1 = []
#data1.append(get_roi_signal(filelist[0]))
'''
for x in range(1,len(filelist)):
    if x != 1:
        data1.append(get_roi_signal(filelist[x]))
    else:
        data1.append(get_roi_signal(filelist[x])[2428:])
#print(data1.shape)
#data2 = data1.reshape(data1.shape[0],int(data1.shape[1]/len(filelist)),len(filelist))
data1 = np.array(data1).transpose(1,2,0)
#print(data1.shape)
data2 = np.concatenate((get_node_network_feature(2428), data1), 2)
#print(data2.shape)
scaler = StandardScaler()
data2 = data2.reshape(2428,1160)
data2 = scaler.fit_transform(data2)
#print(data2)
data2 = data2.reshape(2428,116,10)
'''
#X, Y = getconn_vector(subject_IDs, "correlation", "ho")
#X = get_conn("extracted/global_GretnaSFCMatrixZ")
#X = np.load("feature_z_200.npy")
X = np.load("cc_smriz.npy")
abs_x = map(abs, X)
adjs = np.array(list(abs_x))
#feature = np.load('Craddock_200_connectivity.npy')
feature = get_conn("global_GretnaSFCMatrixZ_Craddock200")
...
adjs = np.delete(adjs,ex_list,axis=0)
...
placeholders = {
    ...
    'features': tf.placeholder(tf.float32, shape=(None, FLAGS.node_num, FLAGS.feature_dim)),
...
for k in range(10):
    ...
    bestModelSavePath0 = 'fold_e_mask%s/gat_e%s_weights_best.ckpt' % (str(k), str(k))
    bestModelSavePath1 = 'fold_e_mask%s/gat_e%s_weights_best.ckpt' % (str(k), str(k))
    ...
    checkpoint = ModelCheckpoint(bestModelSavePath0, monitor='test_acc', verbose=1, save_best_only=True, mode='auto')
    ...
    m_saver.restore(sess,bestModelSavePath0)
    ...
    final_sen.append(sensitivity)
    final_spec.append(specificity)
    final_auc.append(roc_auc)
    final_f1.append(fscore)


print("average accuracy: "+str(np.mean(final_acc)))
print("var:"+str(np.var(final_acc)))
print("max:"+str(np.max(final_acc)))
print("min:"+str(np.min(final_acc)))
print("average sensitivity: "+str(np.mean(final_sen)))
print("var:"+str(np.var(final_sen)))
print("max:"+str(np.max(final_sen)))
print("min:"+str(np.min(final_sen)))
print("average specificity: "+str(np.mean(final_spec)))
print("var:"+str(np.var(final_spec)))
print("max:"+str(np.max(final_spec)))
print("min:"+str(np.min(final_spec)))
print("auc accuracy: "+str(np.mean(final_auc)))
print("var:"+str(np.var(final_auc)))
print("max:"+str(np.max(final_auc)))
print("min:"+str(np.min(final_auc)))
print("average final_f1 score:"+str(np.mean(final_f1)))
print("var:"+str(np.var(final_f1)))
print("max:"+str(np.max(final_f1)))
print("min:"+str(np.min(final_f1)))
final_result = [(np.mean(final_acc),np.var(final_acc),np.mean(final_sen),np.var(final_sen),
          np.mean(final_spec),np.var(final_spec),np.mean(final_auc),np.var(final_auc),np.mean(final_f1),np.var(final_f1))]
analyse_table = pd.DataFrame(final_result, columns=['accuracy','acc var', 'sensitivity', 'sensitivity var',
                                                   'specificity', 'specificity var','auc', 'auc var','f1', 'f1 var',
                                                   ])
analyse_table.to_csv('exp_result_gat.csv')
